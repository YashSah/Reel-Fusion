...
class ProfileController extends GetxController {
  ...
  followUser() async {
    ...
    if(!doc.exists){
      ...
      await firestore.collection('users').doc(_uid.value).collection('following').doc(_uid.value).set({});
      ...
      await firestore.collection('users').doc(_uid.value).collection('following').doc(_uid.value).delete();
      _user.value.update('followers', (value) => (int.parse(value) - 1).toString(),
      );
    }
